...
Although converge of the MCMC is guaranteed in theory, in practice, this is not always the case. Monitoring convergence is crucial whenever MCMC is utilized, to ensure that the recovered results are reliable. 

Depending on the used model, initial values, amount of data, among other things can cause convergence issues. Above, we mentioned two common complications and here we will list a few more, along with actions that can alleviate the issues.


1. Slow convergence. Initial values of the chain are far from most of the target mass, and early iteration bias the approximation. Another cause for slow convergence is that the proposals are not distinct enough from the current value, and the sampler moves too slowly.

2. Incomplete exploration of the target distribution. This means that the sampler doesn't spend enough time at all significant posterior areas. 
...
4. Samples are autocorrelated, meaning that consecutive samples are close to each other. Ideally, we'd like to generate independent samples from the target. High sample autocorrelation can be caused by several factors, including the ones mentioned in the previous points.  
...
2. Discarding an early proportion of the chain as warmup (burn-in).
...
Besides statistics, visually evaluation of the sample can be useful. *Trace plots* refer to graphs where the marginal posterior samples are plotted against sample number. Trace plots can be used to investigate convergence and mixing properties, and can reveal, for example, multimodality. 
