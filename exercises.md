...
```{.error}
Error: 'data/basketball.csv' does not exist in current working directory ('/home/runner/work/statistical-probabilistic-programming-r/statistical-probabilistic-programming-r/site/built').
```
...
```{.error}
Error in `filter()`:
ℹ In argument: `!is.na(FTr)`.
Caused by error:
! object 'FTr' not found
```
