...
To be able to utilize the package functions, we need to add a log-likelihood computation in the Stan code, in the generated quantities block. The generated quantities block computes the pointwise log-likelihood, stored in a variable named `log_lik`. CmdStanR automatically recognizes this variable, allowing the loo package to compute approximate leave-one-out cross-validation. Below, we demonstrate this with the two models we are comparing.
...
``` r
# Fit normal model
normal_fit <- normal_model_loo$sample(
                       data = list(N = N, X = df5$X), 
                       refresh = 0, 
                       seed = 2024
                       )
# Fit cauchy model
cauchy_fit <- cauchy_model_loo$sample(
                       data = list(N = N, X = df5$X), 
                       refresh = 0, 
                       seed = 2024
                       )
...
``` output
 ...
 model2       0.0     0.0      NA                         
 model1     -82.3    36.4    0.99   N < 100 1 k_psis > 0.7
```
...
First we need to extract the log-likelihood values from the fitted model object using `draws()`.


``` r
# Extracting loglik
normal_loglik <- normal_fit$draws("log_lik", format = "matrix")
cauchy_loglik <- cauchy_fit$draws("log_lik", format = "matrix")
...
set.seed(2024)
X_rep <- cauchy_fit$draws("X_rep", format = "matrix")

N_rep <- 9

selected_samples <- sample(1:nrow(X_rep), size = N_rep, replace = FALSE)
X_rep_sub <- X_rep[selected_samples, ]
```
...
Models are specified using familiar R formula syntax. Throughout this lesson, `brms` uses `CmdStanR` as the backend to compile and fit the corresponding Stan models.
...
# Fitting the model
fit_cox <- brms::brm(
  time | cens(1 - status) ~ sex + age + ph.karno_status,
  data = lung,
...
Intercept               1.27      0.68    -0.06     2.57 1.00     3410     2831
sex                    -0.51      0.17    -0.83    -0.19 1.00     3155     2579
age                     0.01      0.01    -0.01     0.03 1.00     3691     2925
ph.karno_statushigh    -0.36      0.18    -0.70     0.00 1.00     2931     2453
...
Intercept           3.5449935  1.966953 0.9441448 13.0026137
sex                 0.6028369  1.182707 0.4348941  0.8310266
age                 1.0131731  1.009298 0.9947433  1.0315701
ph.karno_statushigh 0.6960354  1.194420 0.4949508  1.0021477
```
...
The population-level effects have the normal prior we specified. In `brms`, the default prior for the intercept is Student's t-distribution with three degrees of freedom. The Stan program generated by `brms` can be inspected using `brms::stancode()`. When `backend = "cmdstanr"` is used, this Stan program is compiled and sampled with CmdStan.
...
# Fit model
fit_hier <- brms::brm(
  r2 ~ Anger + Gender + btype + situ + (1 | id),
  family = bernoulli,
  data = VerbAgg,
  backend = "cmdstanr",
  seed = 2024,
...
~id (Number of levels: 316) 
              Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
sd(Intercept)     1.30      0.07     1.17     1.44 1.00      987     1615

Regression Coefficients:
           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
Intercept      0.20      0.33    -0.45     0.85 1.00      819     1517
Anger          0.06      0.02     0.02     0.09 1.00      797     1485
GenderM        0.31      0.18    -0.05     0.69 1.00      670     1357
btypescold    -1.03      0.07    -1.17    -0.90 1.00     5190     3467
btypeshout    -2.00      0.07    -2.14    -1.85 1.00     4539     3071
situself      -1.01      0.06    -1.12    -0.89 1.00     6186     2961
...
plots <- suppressMessages(
  plot(brms::conditional_effects(fit_hier), plot = FALSE))
suppressMessages(
...
fit_hier2 <- update(fit_hier, formula. = ~ . + (1|item), newdata = VerbAgg, seed = 2024,
                    cores = 4, silent = 2, refresh = 0, backend = "cmdstanr")
# Summary
summary(fit_hier2)
```
...
~id (Number of levels: 316) 
              Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
sd(Intercept)     1.37      0.07     1.24     1.51 1.00     1307     1834

~item (Number of levels: 24) 
              Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
sd(Intercept)     0.59      0.11     0.42     0.83 1.00     1327     2222

Regression Coefficients:
           Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
Intercept      0.20      0.44    -0.68     1.04 1.01      964     1363
Anger          0.06      0.02     0.02     0.09 1.01      935     1398
GenderM        0.32      0.20    -0.06     0.72 1.00      858     1487
btypescold    -1.06      0.30    -1.65    -0.45 1.00     1235     1617
btypeshout    -2.11      0.31    -2.72    -1.50 1.00     1206     1931
situself      -1.06      0.25    -1.56    -0.59 1.00     1313     1998
...
Computed from 4000 by 7584 log-likelihood matrix.
         ...
         Estimate   SE
elpd_loo  -4004.5 42.9
p_loo       267.8  3.7
looic      8008.9 85.8
------
MCSE of elpd_loo is 0.2.
MCSE and ESS estimates assume MCMC draws (r_eff in [1.1, 2.3]).
...
Computed from 4000 by 7584 log-likelihood matrix.
         ...
         Estimate   SE
elpd_loo  -3866.3 43.9
p_loo       286.7  4.1
looic      7732.6 87.8
------
MCSE of elpd_loo is 0.2.
MCSE and ESS estimates assume MCMC draws (r_eff in [1.2, 2.9]).
...
Model comparisons:
     model elpd_diff se_diff p_worse diag_diff diag_elpd
 fit_hier2       0.0     0.0      NA                    
  fit_hier    -138.2    16.2    1.00                    
```
...
In addition to the packages covered here, there are several other packages that take advantage of Stan. Here we will briefly introduce some of them.

[rstanarm](https://mc-stan.org/rstanarm/) provides an interface for fitting common Bayesian regression models using Stan. It allows users to specify models using familiar R modeling syntax. Unlike `brms`, which generates Stan code based on the user-defined model formula, `rstanarm` uses a collection of pre-compiled Stan models.

[shinystan](https://mc-stan.org/shinystan/) uses Shiny to provide interactive, customizable visual and numerical summaries of model parameters and convergence diagnostics.

[projpred](https://mc-stan.org/projpred/) performs projection predictive variable selection for Bayesian models. It can be used with models fitted using packages such as `brms` and `rstanarm`.

[posterior](https://mc-stan.org/posterior/) provides tools for manipulating posterior draws and performing common operations such as subsetting, combining, summarizing, and diagnosing posterior distributions. 
