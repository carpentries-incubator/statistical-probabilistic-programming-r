...
Let's quantify the discrepancy using the maximum of the data as a test statistic. The maximum of the original data is max($X$) = 92.469. The following histogram shows this value (vertical line) against the maximum compute for each replicate data set $\tilde{X}$. 
