...
This summary is returned as a data frame, which allows you to easily subset or manipulate the rows and columns using tidyverse tools.

Often, however, it is necessary to process the individual samples. These can be extracted as a matrix using the $draws() from `cmdstanr` method:
...
Modifying the data block so that it declares the hyperparameters as data (e.g. `real<lower=0> alpha;` and `real<lower=0> beta;`) allows the prior parameters to be specified from R rather than hard-coded in the Stan program. This makes it possible to change prior assumptions without modifying the Stan file, improving flexibility and reproducibility when comparing different prior settings.
...
Generate data from a linear model and use the Stan program to estimate the intercept $\alpha$, slope $\beta$, and noise term $\sigma$.
