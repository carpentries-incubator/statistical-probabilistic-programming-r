...
write.csv(df5, "data/df5.csv", row.names = FALSE)
...
```{r, fig.width=15, fig.height=10, echo = FALSE, results = "hide", message = FALSE}
cauchy_fit <- cauchy_model$sample(data = list(N = N, X = df5$X), 
                       refresh = 0)

X_rep <- cauchy_fit$draws("X_rep") %>%
  posterior::as_draws_matrix() %>%
  data.frame() %>%
  mutate(sample = 1:nrow(.))
