...
#### Generate posterior plot using the grid approximation

## Grid approximation **************** ####

delta <- 0.05
df <- expand.grid(seq(-4, 4, by = delta),
                  seq(-4, 5, by = delta)) %>% set_colnames(c("theta1", "theta2"))


for(i in 1:nrow(df)) {
  df[i, "likelihood"] <- prod(
    dnorm(X,
          df[i, "theta1"] + df[i, "theta2"]^2,
          1)
    )
}
